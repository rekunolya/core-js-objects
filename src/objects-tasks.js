...
function removeProperties(obj, keys) {
  ...
  keys.forEach((prop) => delete clone[prop]);
  return clone;
}
...
function makeWord(lettersObject) {
  ...
  const string = new Array(values.length);
  Object.entries(lettersObject).map(([key, value]) => {
    value.forEach((el) => {
      string[el] = key;
    });
    return string;
...
function Rectangle(width, height) {
  class RectangleTask {
    constructor(widthConstr, heightConstr) {
      this.width = widthConstr;
      this.height = heightConstr;
    }
  ...
  return new RectangleTask(width, height);
}
...
function sortCitiesArray(arr) {
  const array = arr.sort((obj1, obj2) => {
    const int = obj1.country.localeCompare(obj2.country);
    if (int === 0) {
